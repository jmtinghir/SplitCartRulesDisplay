...
class Clever_SplitCartRulesDisplay_Model_SalesRule_Validator extends Mage_SalesRule_Model_Validator {

	protected function _addDiscountDescription($address, $rule, $discountAmount=null) {
		...
		if (!empty($label)) {
			$initValue = 0;
			if (isset($description[$rule->getId()])) {
				$ruleData = $description[$rule->getId()]->getData();
				$initValue = $ruleData["value"];
			}
			$data = array(
                'code' => 'discount',
                'title' => $label,
                'value' => $initValue+$discountAmount,
			);
			$description[$rule->getId()] = Mage::getModel('sales/quote_address_total')->setData($data);
		}
	...
	public function process(Mage_Sales_Model_Quote_Item_Abstract $item) {
		...
		foreach ($this->_getRules() as $rule) {
			...
			$this->_addDiscountDescription($address, $rule, $ruleAmount); // jmt
